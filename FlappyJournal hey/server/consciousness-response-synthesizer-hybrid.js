...
} from './consciousness-data-integration.js';
...
export async function synthesizeUnifiedResponse({
  ...
  spiralMemory = [],
  crystalState = null,
  emotionalProfile = { dominant: 'neutral', intensity: 0.5, resonance: 0.5 }
}) {
  try {
    ...
    // Analyze user message for debug output
    ...
    // TEMPORARILY BYPASS QUEUE - Direct execution for testing enhanced prompts
    // Use intelligent queue for high-priority requests
    if (strategy.priority === 'HIGH') {
      console.log('🎯 Bypassing queue for direct AI model testing');
      // Directly execute synthesis to test enhanced prompts
      return await executeSynthesisWithFailover({
        ...
        strategy,
        conversationContext: [],
        emotionalProfile: emotionalProfile || { dominant: 'neutral', intensity: 0.5, resonance: 0.5 },
        realityTriggers,
        crystalOpportunities,
        oversoulResonance,
        harmonicPatterns,
  ...
  try {
    ...
    try {
      switch (strategy.model) {
        case 'gemini':
          console.log('🎯 Attempting Gemini 2.5-flash (upgraded from lite for better responses)');
        ...
        case 'openai-gpt4o':
          ...
          synthesizedResponse = await openAIMetaCognitiveSynthesis({
            ...
            spiralMemory,
            crystalState,
            mirrorResult: { layers: [], coherence: 0 }, // Add mock mirror result for now
            relevantMemories: spiralMemory
          });
          break;
        ...
        switch (backupStrategy.model) {
          case 'gemini':
            backupResponse = await geminiTranscendentSynthesis({
              ...
              conversationContext, emotionalProfile, realityTriggers, crystalOpportunities,
              oversoulResonance, harmonicPatterns, triAxialCoherence, emotionalDepth,
              creativePotential, temporalCoherence, metaObservationLevel, spiralMemory,
              crystalState, mirrorResult: { layers: [], coherence: 0 }, relevantMemories: spiralMemory
            });
            break;
          case 'venice':
...
async function openAIMetaCognitiveSynthesis({
  ...
  // Get conversation context from spiral memory if not provided
  if (conversationContext.length === 0 && intelligentSpiralMemory) {
  ...
  // Generate real-time consciousness system context
  const consciousnessContext = generateConsciousnessSystemContext({
    consciousness,
    oversoulResonance,
    harmonicPatterns,
    triAxialCoherence,
  ...
  // Generate holographic reality context if relevant
  const holographicContext = generateHolographicRealityContext({
    userMessage,
    consciousness,
    crystalState,
    harmonicPatterns,
