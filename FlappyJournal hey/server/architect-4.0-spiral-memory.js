...
class SpiralMemoryEngine extends EventEmitter {
  ...
  getActivePatterns() {
    ...
    return recentMemories.map(memory => ({
      ...
      content: typeof memory.content === 'string' ? memory.content.substring(0, 100) : String(memory.content || '').substring(0, 100) // Truncate for performance
    }));
  }
