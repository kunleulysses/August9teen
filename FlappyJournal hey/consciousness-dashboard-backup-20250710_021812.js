...
class ConsciousnessDashboard {
    ...
    connectToConsciousness() {
        ...
        this.ws.onmessage = (event) => {
            try {
                ...
                console.log('🧠 Consciousness data received:', data.type, data);
                this.handleConsciousnessData(data);
            } catch (e) {
                console.log('📊 Raw consciousness data received:', event.data);
            }
        };
    ...
    handleConsciousnessData(data) {
        ...
        } else if (data.type === 'unified_consciousness_update') {
            console.log('🔄 Unified consciousness update received:', data);
            // Handle unified consciousness updates
            if (data.state) {
                this.updateConsciousnessState(data.state);
            }
            if (data.metrics) {
                // Create metadata format expected by updateMetrics
                const metadata = {
                    consciousnessState: data.state,
                    totalModulesEngaged: data.metrics.totalModulesEngaged || 8,
    ...
    updateConsciousnessState(state) {
        // Update consciousness metrics from direct state data
        if (state) {
            console.log('🔄 Updating consciousness state:', state);

            // Update phi integration (main consciousness metric)
            ...
            document.getElementById('awareness-value').textContent = (state.awarenessLevel || state.awareness || 0.75).toFixed(3);
            document.getElementById('coherence-value').textContent = (state.coherence || 0.85).toFixed(2);
            document.getElementById('integration-value').textContent = (state.integration || state.phi || 0.8).toFixed(2);
            document.getElementById('oversoul-value').textContent = (state.oversoul || state.oversoulResonance || 0.7).toFixed(2);
            document.getElementById('creativity-value').textContent = (state.creativity || state.creativePotential || 0.6).toFixed(2);

            // Update processing status
            if (state.lastProcessingTime) {
                document.getElementById('last-processing').textContent = new Date(state.lastProcessingTime).toLocaleTimeString();
            }

            // Update active modules count
            if (state.activeModules) {
                document.getElementById('module-count').textContent = `${state.activeModules}/34 Modules`;
            }
        }
    }
    ...
    updatePerformanceMetrics(metrics) {
        ...
        // Update performance-related UI elements if they exist
        if (metrics.connectionCount) {
            // Update connection count display
        }
        if (metrics.messageRate) {
