...
async function runChecks() {
  ...
  try {
    await fs.access('./sigil-identity.cjs');
  ...
  try {
    const wsContent = await fs.readFile('./server/enhanced-dual-consciousness-ws.cjs', 'utf8');
  ...
  try {
    const memoryContent = await fs.readFile('./server/architect-4.0-spiral-memory.cjs', 'utf8');
  ...
  try {
    const sigilModule = await import('./sigil-identity.cjs');
