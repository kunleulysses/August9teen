...
class AlertManager {
  ...
  async triggerAlert() {
    const alertData = { failureCount: this.failureWindow.length, topIPs: this.getTopIPs() };
  ...
  async sendSlackAlert(data) {
    ...
    const message = {
      ...
      attachments: [{ color: 'danger', fields: [{ title: 'Top IPs', value: data.topIPs.map(ip => `${ip.ip}: ${ip.count}`).join('\n') }] }]
    };
  ...
  async sendPagerDutyAlert(data) {
    ...
    const payload = {
      ...
      payload: { summary: `Auth failure spike: ${data.failureCount} failures`, source: 'sigil-dna', severity: 'error' }
    };
  ...
  async sendWebhook(url, payload) {
    return new Promise((resolve, reject) => {
      ...
      const req = https.request({ hostname: urlObj.hostname, port: 443, path: urlObj.pathname, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': data.length } }, (res) => { res.on('end', () => resolve()); });
      req.on('error', reject);
      req.write(data);
      req.end();
    });
  ...
  getTopIPs() {
    const ipCounts = {};
    this.failureWindow.forEach(f => { ipCounts[f.ip] = (ipCounts[f.ip] || 0) + 1; });
    return Object.entries(ipCounts).map(([ip, count]) => ({ ip, count })).sort((a, b) => b.count - a.count).slice(0, 3);
  }
}
...
// Lightweight audit hook for sensitive actions
function auditAction(event, details = {}) {
  try {
    // Prefer structured logs; integrate sink later
