...
class SigilEngine {
  ...
  async decode(sigilId, options = {}) {
    ...
    // Try to get record directly if storage supports it
    let record;
    try {
    ...
    return {
      success: true,
      data: record.data,
      consciousnessState: record.consciousnessState,
      metadata: {
        sigilId: record.id,
  ...
  async verify(data, signature, options = {}) {
    ...
    if (!data || !signature) {
      throw new Error('Data and signature required for verification');
    }
  ...
  async revoke(sigilId, options = {}) {
    ...
    if (!sigilId) {
      throw new Error('SigilId required for revocation');
    }
  ...
  async list(options = {}) {
    ...
    return {
      sigils: records.map(r => ({
        id: r.id,
        timestamp: r.timestamp,
        consciousnessState: r.consciousnessState,
        resonancePattern: r.resonancePattern
      })),
      total,
      tenantId,
      pagination: {
  ...
  _calculateResonancePattern(data, consciousnessState) {
    ...
    // Generate resonance vector based on data and consciousness
  ...
  async getHealth() {
    ...
    return {
      ...
      config: {
        enableResonance: this.config.enableResonance,
        maxSigilSize: this.config.maxSigilSize
      },
      timestamp: new Date().toISOString()
    };
  }
