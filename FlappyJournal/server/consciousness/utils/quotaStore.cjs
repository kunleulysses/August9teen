 ...
 */
async function incrWithinHour(id = 'default', step = 1, maxAllowed = DEFAULT_MAX) {
  await store.ready;
  const now = Date.now();
...
async function resetQuota(id = 'default') {
  await store.ready;
  await store.setQuota(id, 0, Date.now() + HOUR_MS);
}

async function getQuota(id = 'default') {
  return await store.getQuota(id);
}

module.exports = { incrWithinHour, resetQuota, getQuota };
