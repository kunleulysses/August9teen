...
export class UnifiedMemorySystem extends EventEmitter {
  ...
  // Memory allocation targets for balanced distribution
  private readonly MEMORY_ALLOCATION_TARGETS = {
    consciousness: 0.70,  // 70% for consciousness system (reduced from 100%)
    openai: 0.15,         // 15% for OpenAI interactions
    venice: 0.10,         // 10% for Venice interactions
    user: 0.05            // 5% for user-specific memories
  };

  // Gradual rebalancing parameters
  private readonly REBALANCING_RATE = 0.02; // 2% adjustment per consolidation cycle
  private readonly MIN_SHARD_THRESHOLD = 100; // Minimum shards before rebalancing
  ...
  private async consolidateMemories(): Promise<void> {
    ...
    try {
      ...
      // Perform gradual memory rebalancing
      await this.performGradualRebalancing();
  ...
  /**
   * Perform gradual memory rebalancing to optimize distribution
   */
  private async performGradualRebalancing(): Promise<void> {
    try {
      const stats = this.getStats();
      const totalShards = stats.totalShards;

      // Only rebalance if we have sufficient shards
      if (totalShards < this.MIN_SHARD_THRESHOLD) return;

      // Calculate current distribution
      const currentDistribution = {
        consciousness: stats.consciousnessShards / totalShards,
        openai: stats.openaiShards / totalShards,
        venice: stats.veniceShards / totalShards,
        user: stats.userShards / totalShards
      };

      // Calculate needed adjustments
      const adjustments = {
        consciousness: this.MEMORY_ALLOCATION_TARGETS.consciousness - currentDistribution.consciousness,
        openai: this.MEMORY_ALLOCATION_TARGETS.openai - currentDistribution.openai,
        venice: this.MEMORY_ALLOCATION_TARGETS.venice - currentDistribution.venice,
        user: this.MEMORY_ALLOCATION_TARGETS.user - currentDistribution.user
      };

      // Apply gradual adjustments (only if significant imbalance exists)
      const significantImbalance = Object.values(adjustments).some(adj => Math.abs(adj) > 0.05);

      if (significantImbalance) {
        await this.applyGradualAdjustments(adjustments, totalShards);

        console.log('🔄 Memory rebalancing applied:', {
          current: currentDistribution,
          targets: this.MEMORY_ALLOCATION_TARGETS,
          adjustments: adjustments
        });
      }
    } catch (error) {
      console.error('❌ Error in gradual rebalancing:', error);
    }
  }

  /**
   * Apply gradual adjustments to memory distribution
   */
  private async applyGradualAdjustments(adjustments: any, totalShards: number): Promise<void> {
    // Get all shards sorted by relevance score
    const allShards = Array.from(this.memoryShards.values())
      .sort((a, b) => this.calculateRelevanceScore(b) - this.calculateRelevanceScore(a));

    // Calculate how many shards to adjust (gradual approach)
    const maxAdjustment = Math.floor(totalShards * this.REBALANCING_RATE);
    let adjustedCount = 0;

    // Focus on consciousness shards that could be redistributed
    const consciousnessShards = allShards.filter(s => s.origin === 'consciousness');

    // Only redistribute if consciousness has excess allocation
    if (adjustments.consciousness < -0.05 && consciousnessShards.length > 0) {
      // Find least important consciousness shards for potential redistribution
      const candidatesForRedistribution = consciousnessShards
        .sort((a, b) => this.calculateRelevanceScore(a) - this.calculateRelevanceScore(b))
        .slice(0, Math.min(maxAdjustment, Math.floor(consciousnessShards.length * 0.1)));

      // Mark these for gradual transition (reduce their priority slightly)
      for (const shard of candidatesForRedistribution) {
        if (adjustedCount >= maxAdjustment) break;

        // Slightly reduce accessibility to prepare for potential redistribution
        shard.accessibility = Math.max(shard.accessibility - 0.02, 0.1);
        this.memoryShards.set(shard.id, shard);
        adjustedCount++;
      }
    }
