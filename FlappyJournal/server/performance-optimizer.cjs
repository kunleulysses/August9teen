...
class PerformanceOptimizer extends EventEmitter {
  ...
  optimizeMessage(message, clientId) {
    ...
    // Debug logging for message priority
    console.log(`🔍 Message optimization: type="${messageType}", priority="${priority}"`);

    // Add to batch for non-high priority messages
    if (priority !== 'HIGH') {
      console.log(`📦 Batching message: ${messageType} (priority: ${priority})`);
      this.addToBatch(clientId, message, priority);
      return null; // Return null to indicate message was batched
    }

    console.log(`⚡ Processing immediately: ${messageType} (HIGH priority)`);
