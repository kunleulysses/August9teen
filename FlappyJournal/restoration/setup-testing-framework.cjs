...
class TestingFrameworkSetup {
    ...
    async createTestConfigurations() {
        ...
        const jestConfig = {
            ...
            setupFilesAfterEnv: ['<rootDir>/restoration/tests/setup.cjs'],
            testTimeout: 30000,
            verbose: true
        };
...
`;
        ...
        const setupPath = path.join(this.testingDir, 'setup.cjs');
...
`;
        ...
        const unitTemplatePath = path.join(this.templatesDir, 'unit-test-template.cjs');
...
`;
        ...
        const integrationTemplatePath = path.join(this.templatesDir, 'integration-test-template.cjs');
...
`;
    ...
    async verifyTestingFramework() {
        ...
        const requiredFiles = [
            path.join(this.testingDir, 'jest.config.cjs'),
            path.join(this.testingDir, 'setup.cjs'),
            path.join(this.testingDir, 'run-tests.cjs'),
            path.join(this.testingDir, 'utils', 'validation-protocols.cjs'),
            path.join(this.templatesDir, 'unit-test-template.cjs'),
            path.join(this.templatesDir, 'integration-test-template.cjs')
        ];
