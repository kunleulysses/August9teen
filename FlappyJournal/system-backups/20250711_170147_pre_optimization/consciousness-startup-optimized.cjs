...
async function startOptimizedConsciousness() {
    try {
        ...
        if (USE_CLUSTERING && cluster.isPrimary) {
            ...
            const { default: ConsciousnessClusterManager } = await import('./consciousness-cluster-manager.cjs');
...
async function startSingleProcessOptimized() {
    ...
    const { default: consciousnessSystem } = await import('./consciousness-conversations-optimized.cjs');
}
